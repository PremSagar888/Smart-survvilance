...
if (isLocal) {
    // If running on the local PC, always connect to the local loopback WebSocket server
    initialWsAddress = `ws://127.0.0.1:8765`;
} else if (brokerParam) {
...
function attachEventListeners() {
    ...
    if (mobileMenuToggle && sidebarCollapsible) {
        mobileMenuToggle.addEventListener('click', () => {
            const isHidden = sidebarCollapsible.classList.contains('hidden');
            if (isHidden) {
                sidebarCollapsible.classList.remove('hidden');
                if (mobileMenuIcon) {
                    mobileMenuIcon.setAttribute('d', 'M6 18L18 6M6 6l12 12'); // X icon
                }
            } else {
                sidebarCollapsible.classList.add('hidden');
                if (mobileMenuIcon) {
                    mobileMenuIcon.setAttribute('d', 'M4 6h16M4 12h16m-7 6h7'); // Hamburger icon
                }
            }
        });
    }
...
// ========== RESET DASHBOARD UI FOR NEW SESSION ==========
function resetDashboardUI() {
    ...
    // Reset Stats
    const framesCount = document.getElementById('total-frames-count');
    const personsCount = document.getElementById('unique-persons-count');
    const avgConfidence = document.getElementById('avg-confidence');
    if (framesCount) framesCount.textContent = '0';
    if (personsCount) personsCount.textContent = '0';
    if (avgConfidence) avgConfidence.textContent = '0.0%';
    ...
    if (statusDesc) statusDesc.textContent = 'No suspicious activity detected';
    if (statusBanner) statusBanner.className = 'alert-banner alert-normal';
    if (bannerText) bannerText.textContent = 'Surveillance Active - System Monitoring Healthy';

    // Reset Suspects
    const suspectsGrid = document.getElementById('suspects-grid');
    const suspectsCount = document.getElementById('suspects-count');
    if (suspectsCount) suspectsCount.textContent = '0 Suspects';
    if (suspectsGrid) {
        suspectsGrid.innerHTML = `
            <div class="text-center py-8 text-[#A7ADB5] border border-dashed border-[#31363F] rounded-lg text-xs">
                No suspects captured.
    ...
    // Reset Qwen narrative
    const qwenText = document.getElementById('qwen-summary-text');
    if (qwenText) {
        qwenText.textContent = '"Awaiting surveillance stream compilation. Factual narrative reports generated by Qwen model will populate here automatically upon session complete."';
    }
...
function connectWebSocket() {
    ...
    try {
        ...
        state.websocket.onopen = function () {
            ...
            // Re-enable and reset buttons on a fresh connection
            const stopBtn = document.getElementById('stop-updates');
            const toggleBtn = document.getElementById('toggle-updates');
            if (stopBtn) {
                stopBtn.disabled = false;
                stopBtn.textContent = "Stop Live";
                stopBtn.className = "flex-grow px-4 py-3 bg-[#E63946] hover:bg-[#D62839] text-white rounded-lg transition font-semibold text-xs whitespace-nowrap";
            }
            if (toggleBtn) {
                toggleBtn.disabled = false;
                toggleBtn.textContent = "Pause Live";
                toggleBtn.className = "flex-grow px-4 py-3 bg-[#31363F] hover:bg-[#3F444E] text-[#A7ADB5] rounded-lg transition font-semibold text-xs whitespace-nowrap";
            }
        };
...
function scheduleReconnect() {
    ...
    state.reconnectTimer = setTimeout(() => {
        ...
        if (!isLocal && !brokerParam && !wssParam) {
            fetch('session.json?t=' + new Date().getTime())
                .then(res => res.json())
                .then(data => {
                    if (data && data.ws_url) {
                        state.wsAddress = data.ws_url.includes('?') ? data.ws_url : `${data.ws_url}?role=frontend`;
                        console.log("Updated WebSocket address from session.json:", state.wsAddress);
                    }
                    connectWebSocket();
                })
                .catch(err => {
                    console.log("Failed to fetch session.json on reconnect, using:", state.wsAddress);
                    connectWebSocket();
                });
        } else {
            connectWebSocket();
...
function updateConnectionStatus(connected) {
    ...
    let warningEl = document.getElementById('mixed-content-warning');
    ...
    if (connected) {
        ...
        if (window.location.protocol === 'https:' && ipParam) {
            if (!warningEl) {
                warningEl = document.createElement('div');
                warningEl.id = 'mixed-content-warning';
                warningEl.className = 'mt-3 p-3 bg-red-950/40 border border-red-500/30 rounded text-[11px] text-red-200 leading-normal space-y-2';
                
                const parent = statusText.closest('.bg-\\[\\#1A1E23\\]');
                if (parent) {
                    parent.appendChild(warningEl);
                }
            }
            warningEl.style.display = 'block';
            const httpUrl = `http://${ipParam}:8000/dashboard/index.html?ip=${ipParam}`;
            warningEl.innerHTML = `
                <p class="font-semibold text-red-400">⚠️ Connection Blocked (HTTPS)</p>
                <p>Mobile browsers block local WebSocket connections on HTTPS pages.</p>
                <a href="${httpUrl}" class="inline-block mt-1.5 font-bold text-[#C97C2C] hover:underline">
                    👉 Click to open HTTP Version
                </a>
            `;
        }
    }
...
function processLiveMessage(rawMessage) {
    try {
        const delimiter = '|';
        ...
        if (type === 'STATS') {
            const totalFrames = parts[1];
            const persons = parts[2];
            const objects = parts[3];
            const avgConf = parts[4];
            
            document.getElementById('total-frames-count').textContent = totalFrames;
